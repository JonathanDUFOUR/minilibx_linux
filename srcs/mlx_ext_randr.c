...
/*   Updated: 2021/07/28 03:03:34 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mlx_ext_fullscreen(t_xvar *xvar, t_win_list *win, int fullscreen)
{
	...
	if (!fullscreen)
		mode_candidate = g_saved_mode;
	crtc = XRRGetCrtcInfo(xvar->display, res, o_info->crtc);
	g_saved_mode = crtc->mode;
	i = XRRSetCrtcConfig(xvar->display,
			res,
			o_info->crtc,
			CurrentTime,
			0,
